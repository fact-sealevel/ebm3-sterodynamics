...
import os
import pandas as pd
from ebm3_sterodynamics import energy_balance_model as ebm3
import csv
import xarray as xr
...
def fetch_erfs_from_rcmip(path, scenarios):
    ...
    for s, scen in enumerate(scenarios):
        try:
            ...
            raise KeyError(f"Scenario '{scen}' not found in ssp_idx lookup table")
        except IndexError:
            raise IndexError(
                f"Row index {ssp_idx[scen]} out of range for scenario '{scen}' "
                f"(CSV has {len(rows)} rows)"
            )
        except ValueError as e:
            raise ValueError(f"Could not assign ERF data for scenario '{scen}': {e}")
...
def angd(lat0, lon0, qlat, qlon):
    ...
    temp = np.arctan2(
        np.sqrt(
            (np.cos(qlat) * np.sin(qlon - lon0)) ** 2
            + (
                np.cos(lat0) * np.sin(qlat)
                - np.sin(lat0) * np.cos(qlat) * np.cos(qlon - lon0)
            )
            ** 2
        ),
        (
            np.sin(lat0) * np.sin(qlat)
            + np.cos(lat0) * np.cos(qlat) * np.cos(qlon - lon0)
        ),
    )
...
def NearestPoints(qlats, qlons, lats, lons, tol):
    ...
    if len(qlats) != len(qlons):
        raise Exception(
            "Query lats ({}) and lons ({}) differ in length".format(
                len(qlats), len(qlons)
            )
        )

    idx = map(
        lambda qlat, qlon: NearestPoint(qlat, qlon, lats, lons, tol), qlats, qlons
    )
...
def ebm3_oceandynamics_postprocess(
    ...
    gsat = cf["surface_temperature"].sel(years=projyears) - cf[
        "surface_temperature"
    ].sel(years=np.arange(baseyear - 9, baseyear + 10)).mean(dim="years")
    otemp = cf["deep_ocean_temperature"].sel(years=projyears) - cf[
    ...
    scenarios = [
        "ssp126",
        "ssp585",
    ]  # NOT INPUT, these two are needed for SSP interpolation in 2300 projections
    ...
    if pyear_end < 2151:  #        # keep those up to 2300 or 2500
        for m, model in enumerate(model_has_zos):
            ...
            zos_runs2_2100 = [
                f for f in os.listdir(modeldir) if scenario in f
            ]  # regex would be better here
            if not zos_runs2_2100:
                print(" ")
                print(
                    scenario + " scenario not found in model " + model + ". Moving on."
                )
                continue
            print(" ")
            ...
            for v, filename in enumerate(zos_runs2_2100):
                ...
                if not model_var_param.empty:  # if not empty, check if that variant is availble for both hist and ssps
                    ...
                    hist_file = [
                        f for f in os.listdir(modeldir) if "hist" in f and variant in f
                    ]
                    ssp_file = [
                        f
                        for f in os.listdir(modeldir)
                        if scenario in f and variant in f
                    ]
                if (
                    hist_file and ssp_file
                ):  # if there is zos sims for hist and ssps and ebm parameters, continue with emulation
                    break
            ...
            # get forcing
            erfs, erfyears = fetch_erfs_from_rcmip(
                rfmipfile, [scenario]
            )  # get ERF timeseries
            ...
            attrs = {
                "description": "surface and deep temperature obtained by EBM-3LM",
                "contact": "Victor Malagon Santos, victor.malagon.santos@nioz.nl",
            }

            temp3_xr = xr.Dataset(
                {"temperature": (["years", "layers"], temp3)},
                coords={"layers": ["surface", "inter", "deep"], "years": erfyears},
                attrs=attrs,
            )
            ...
            try:
                hist_xr = xr.open_dataset(
                    modeldir + "/" + hist_file[0], decode_times=False
                )
                ssp_xr = xr.open_dataset(
                    modeldir + "/" + ssp_file[0], decode_times=False
                )
            except:
            ...
            zos.coords["years"] = ((zos["time"] // 365.25) + 1850).astype(
                "int"
            )  # REMOVE HARDCODED STARTING YEAR

            # Calculate annual means
            zos_am = zos.groupby("years").mean(dim="time")
            zos_am = zos_am.where(
                zos_am["zos"] < 99999
            )  # replace land values with nans
            ...
            Ts = np.array(
                temp3_xr["temperature"].sel(years=targyears).sel(layers="surface")
            )
            Ti = np.array(
                temp3_xr["temperature"].sel(years=targyears).sel(layers="inter")
            )
            Td = np.array(
                temp3_xr["temperature"].sel(years=targyears).sel(layers="deep")
            )
            ...
            slopes.append(
                slope
            )  # MAYBE SAVE TO A PICKLE, TO BE PROVIDED IN THE NEXT TASK?
    ...
    if pyear_end >= 2151:
        for m, model in enumerate(model_has_zos):
            ...
            zos_runs2_2300 = [
                f for f in os.listdir(modeldir) if "2300" in f or "2500" in f
            ]  # regex would be better here
            if not zos_runs2_2300:
                ...
                print(
                    "Model "
                    + model
                    + " only runs to 2100. Skipping and looking for longer simulations"
                )
                continue
            print(" ")
            ...
            for v, filename in enumerate(zos_runs2_2300):
                ...
                if not model_var_param.empty:  # if not empty, check if that variant is availble for both hist and ssps
                    ...
                    ssp1_file = [
                        f
                        for f in os.listdir(modeldir)
                        if "ssp126" in f and variant in f
                    ]
                    ssp5_file = [
                        f
                        for f in os.listdir(modeldir)
                        if "ssp585" in f and variant in f
                    ]
                if (
                    hist_file and ssp1_file and ssp5_file
                ):  # if there is zos sims for hist and ssps and ebm parameters, continue with emulation
                    break
            ...
            temp3_xr = xr.Dataset(
                {"temperature": (["years", "layers", "scenarios"], temp3)},
                coords={
                    "layers": ["surface", "inter", "deep"],
                    "years": erfyears,
                    "scenarios": scenarios,
                },
                attrs=attrs,
            ...
            try:
                ...
                ssp1_xr = xr.open_dataset(
                    modeldir + "/" + ssp1_file[0], decode_times=False
                )
                ssp5_xr = xr.open_dataset(
                    modeldir + "/" + ssp5_file[0], decode_times=False
                )
            except:
                print(
                    "Model misses one of the scenarios used for emulation. Moving on."
                )
                continue
            ...
            # % create new coordinates for year
            zos1.coords["years"] = ((zos1["time"] // 365.25) + 1850).astype(
                "int"
            )  # REMOVE HARDCODED STARTING YEAR
            zos5.coords["years"] = ((zos5["time"] // 365.25) + 1850).astype(
                "int"
            )  # REMOVE HARDCODED STARTING YEAR

            # Calculate annual means
            zos1_am = zos1.groupby("years").mean(dim="time")
            zos1_am = zos1_am.where(
                zos1_am["zos"] < 99999
            )  # replace land values with nans
            zos5_am = zos5.groupby("years").mean(dim="time")
            zos5_am = zos5_am.where(
                zos5_am["zos"] < 99999
            )  # replace land values with nans
            ...
            Ts = xr.concat(
                (
                    temp3_xr.sel(years=targyears)
                    .sel(scenarios=["ssp126"])
                    .sel(layers="surface")
                    .drop_vars("scenarios"),
                    temp3_xr.sel(years=targyears)
                    .sel(scenarios=["ssp585"])
                    .sel(layers="surface")
                    .drop_vars("scenarios"),
                ),
                dim="years",
            ...
            for i in range(nlat):
                for j in range(nlon):
                    y = np.concatenate(
                        (
                            Smooth(zos1[:, i, j], w=smoothwin),
                            Smooth(zos5[:, i, j], w=smoothwin),
                        )
                    )  # reducing varability
    ...
    for sample in samples:
        ...
        Tfi = np.array(
            otemp.sel(layers=1).sel(samples=sample)
        )  # intermediate temperature from fair
        Tfd = np.array(
            otemp.sel(layers=2).sel(samples=sample)
        )  # deep temperature from fair

        # % project
        dsl[sample, :, :] = (
            np.multiply.outer(Tfs, slope_s)
            + np.multiply.outer(Tfi, slope_i)
            + np.multiply.outer(Tfd, slope_d)
        )
    ...
    sdsl = (
        dsl * 1000
        - np.repeat(
            np.array(weighted_mean["sea_level_change"])[:, :, np.newaxis],
            len(site_ids),
            axis=2,
        )
        + np.repeat(gte[0:nsamps, :, np.newaxis], len(site_ids), axis=2)
    )

    ncvar_attributes = {
        "description": "Local SLR contributions from thermal expansion and dynamic sea-level using EBM3",
...
def ebm3_oceandynamics_project_fn(
    ...
    if lslr_out_file is not None and lslr_quantile_out_file is not None:
        ...
        ebm3_oceandynamics_postprocess(
            scenario,
            pipeline_id,
            nsamps,
            seed,
        ...
        print(
            "Local output files not provided, skipping post-processing and only running global projections."
        )
